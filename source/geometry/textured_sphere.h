...
class TexturedSphere : public Sphere {
 public:
  ...
  TexturedSphere(glm::vec3 origin,
                 ...
                 glm::vec3 emission,
                 glm::vec3 specular,
                 const std::string &filepath, TextureType type) : Sphere(
      origin,
      radius,
      color,
      emission,
      specular) {
    switch (type) {
      case TextureType::DIFFUSE:m_tx_diffuse = std::make_shared<BitmapTexture>(type);
        m_tx_diffuse->Read(filepath.c_str());
        break;
      case TextureType::EMISSIVE:m_tx_emission = std::make_shared<BitmapTexture>(type);
        m_tx_emission->Read(filepath.c_str());
        break;
      case TextureType::SPECULAR:m_tx_specular = std::make_shared<BitmapTexture>(type);
  ...
  TexturedSphere() : Sphere::Sphere(), m_tx_diffuse(std::make_shared<BitmapTexture>(TextureType::NONE)) {}
  ...
  [[nodiscard]] glm::vec3 &GetSpecularColor(glm::vec3 &normal) {
    ...
    } else return m_specular;

  }

  bool addTexture(const std::string &path, TextureType type) {
    switch (type) {
      case TextureType::NONE:return false;
      case TextureType::DIFFUSE:
        ...
        return true;
      case TextureType::EMISSIVE:std::cout << "\n";
        if (m_tx_emission == nullptr) {
...
//  }
 ...
 private:
  ...
  std::shared_ptr<BitmapTexture> m_tx_diffuse, m_tx_emission, m_tx_specular;
};
